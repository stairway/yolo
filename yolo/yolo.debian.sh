...
#     $ YOLO_DATA_TARGET="$PWD/../mount/data/" sh yolo.debian.sh
# Example using multiple overrides:
#     $ YOLO_DATA_TARGET="$PWD/../mount/data/" PORT_EIGHT_THOUSAND=9100 PORT_EIGHTY_EIGHTY=9180 RANDOM=$RANDOM ENTRYPOINT_NAME=yolo.docker-entrypoint.debian.$RANDOM.sh sh yolo.debian.sh
...
PORT_EIGHT_THOUSAND="${PORT_EIGHT_THOUSAND:-9100}"; \
PORT_EIGHTY_EIGHTY=${PORT_EIGHTY_EIGHTY:-9180}; \
SEED="${RANDOM:-$SEED}"; \
...
OS_NAME="\\\$(echo "\\\$OS_RELEASE" | awk '{print \\\$1}')"
OS_ID="\\\$(echo "\\\$OS_RELEASE" | awk '{print \\\$2}')"
OS_VERSION_ID="\\\$(echo "\\\$OS_RELEASE" | awk '{print \\\$3}')"
OS_VERSION_CODENAME="\\\$(echo "\\\$OS_RELEASE" | awk '{print \\\$4}')"
OS_RELEASE="\\\$(echo "\\\${OS_ID}\\\${1:--}\\\${OS_VERSION_CODENAME}")"
...
__install_crypto() {
  ...
  # kerberos requires interactive setup
  # apt-get install -y --upgrade krb5-user sssd-krb5
  __cleanup_deps_caches
}
...
__configure_java() {
  apt-get install -y default-jdk maven # default-jre
  JAVA_PATH=\$(update-alternatives --list java)
  cat <<EOT >"\$PROFILE_DIR/01-java.sh"
# This file was generated by $SCRIPT_FILENAME
# DO NOT EDIT THIS FILE BY HAND -- CHANGES WILL BE OVERWRITTEN
...
if [ -n "\\\$YOLO_DOMAIN" ]; then
  [ -d $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN ] || mkdir -p $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN
  if [ -d $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN ]; then
    ...
    if [ "\\\${key_count}" -lt 1 ]; then
      printf "\033[93m>\033[0m Generating ssh ed25519 keypair with empty password ...\n\033[96;1m%s\033[0m\n" "ssh-keygen -t ed25519 -C '\\\${GIT_CONFIG_EMAIL}' -f $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN/id_ed25519 -N ''"
      ssh-keygen -t ed25519 -C "\\\${GIT_CONFIG_EMAIL}" -f $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN/id_ed25519 -N "" > $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN/id_ed25519.fingerprint && cat $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN/id_ed25519.pub
    fi
    key_count=0
    key_count=\\\$(ls -1 $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN | grep --color=never -o id_rsa.fingerprint | wc -l)
    if [ "\\\${key_count}" -lt 1 ]; then
      printf "\033[93m>\033[0m Generating ssh rsa keypair with empty password ...\n\033[96;1m%s\033[0m\n" "ssh-keygen -t rsa -b 4096 -C '\\\${GIT_CONFIG_EMAIL}' -f $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN/id_rsa -N ''"
      ssh-keygen -t rsa -b 4096 -C "\\\${GIT_CONFIG_EMAIL}" -f $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN/id_rsa -N "" > $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN/id_rsa.fingerprint && cat $SSH_CONFIG_PREFIX/\\\$YOLO_DOMAIN/id_rsa.pub
    fi
    unset key_count
  fi
...
# Quick command
# YOLO_DATA_TARGET=~/Data/Liatrio sh ~/Tools/containerfy/yolo.debian.sh
