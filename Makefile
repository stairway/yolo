...
fix-permissions: ## Fixes permissions for multiple users
	...
	@(set -x; ! test -d .dockermount || find .dockermount -type f -name '*.sh' -mindepth 1 -exec sudo chmod -R 0775 {} \;)
	@(set -x; ! test -d .dockermount || find .dockermount -type f -name '*.sh.*' -mindepth 1 -exec sudo chmod -R 0775 {} \;)
	@(set -x; find . -type f -mindepth 1 -maxdepth 1 ! -name '.*' ! -name '*.sh' -exec sudo chmod -R 0664 {} \;)
	@(set -x; find . -type f -mindepth 1 -maxdepth 1 -name '*.sh' -exec sudo chmod -R 0775 {} \;)
